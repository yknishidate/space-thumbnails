...
//!     status 0 => bytes; 2 => parsed but no renderable geometry; else utf8 error
...
/// Helper status byte for "file parsed but has no renderable geometry".
const STATUS_NO_GEOMETRY: i32 = 2;

/// A successful round-trip to the helper.
pub enum RenderResult {
    /// Raw pixel payload (RGBA8 for the render helper).
    Pixels(Vec<u8>),
    /// The file parsed but held no renderable geometry — not an error, just
    /// nothing to draw (helper status 2).
    Empty,
}
...
impl HelperClient {
    ...
    /// Renders `path` at `size`. Returns `Pixels` (RGBA8 for the render helper)
    /// or `Empty` when the file parsed but had nothing to draw. On any failure
    /// the helper is torn down so the next call starts fresh.
    pub fn render(&self, path: &str, size: u32) -> io::Result<RenderResult> {
        let mut slot = self
            .worker
            .lock()
        ...
        let result = (|| {
            ...
            match response.status {
                0 => Ok(RenderResult::Pixels(response.payload)),
                STATUS_NO_GEOMETRY => Ok(RenderResult::Empty),
                _ => Err(io::Error::new(
                    io::ErrorKind::Other,
