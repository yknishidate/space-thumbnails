...
impl AssimpAsset {
    pub fn from_memory(
        ...
        Self::from_memory_with_flags(engine, buffer, hint, DEFAULT_FLAGS)
    }
    pub fn from_memory_with_flags(
        ...
        unsafe {
            ...
            if ai_scene.is_null() {
                return Err(E::FailedLoadModel(assimp_error_string()));
            }
    ...
    pub fn from_file_with_flags(
        ...
        unsafe {
            ...
            let ai_scene = russimp_sys::aiImportFile(c_filename.as_ptr(), flags as u32);
            if ai_scene.is_null() {
                return Err(E::FailedLoadModel(assimp_error_string()));
            }
...
pub enum AssimpAssetError {
    ...
    EmptyModel { vertices: usize, indices: usize },
    InvalidString,
    InternalError,
}
...
impl fmt::Display for AssimpAssetError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::FailedLoadModel(message) => write!(f, "Failed to load model: {message}"),
            Self::EmptyModel { vertices, indices } => write!(
                f,
                "Imported model has no renderable geometry (vertices: {vertices}, indices: {indices})."
            ),
...
fn assimp_error_string() -> String {
    unsafe {
        let message = russimp_sys::aiGetErrorString();
        if message.is_null() {
            "unknown Assimp error".to_owned()
        } else {
